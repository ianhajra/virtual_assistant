...
def test_process_text():
    ...
    assert process_text("ALL CAPS INPUT") == ["cap", "input"]
    assert process_text("all lowercase input") == ["lowercas", "input"]
    ...
    assert process_text("THIS IS A TEST") == ["test"]
    assert process_text("python CODE") == ["python", "code"]
    assert process_text("12345") == []
    assert process_text("MixedCASE123") == ["mixedcas"]
    assert process_text("Leading and Trailing Spaces    ") == ["lead", "trail", "space"]
    assert process_text("    ALL SPACES IN FRONT") == ["space", "front"]
    assert process_text("SpecialCharacters!@#") == ["specialcharact"]
    assert process_text("CamelCaseWord") == ["camelcaseword"]
    assert process_text("UPPERCASE123lower") == ["uppercaselow"]
    assert process_text("numbers1234567890") == ["number"]
    assert process_text("    SPACES everywhere  ") == ["space", "everywher"]
    assert process_text("String with Tabs\t and Newlines\n") == ["string", "tab", "newlin"]
    assert process_text("UPPER AND lower") == ["upper", "lower"]
    assert process_text("123 Mixed Upper LOWER!") == ["mix", "upper", "lower"]
    assert process_text("Ümläüts ShÖuld BE lowER") == ["mlt", "shuld", "lower"]
    assert process_text("LONG STRING OF TEXT TO TEST THE FUNCTION") == ["long", "string", "text", "test", "function"]
    assert process_text("SPACES\tTABS\nNEWLINES") == ["space", "tab", "newlin"]
    assert process_text("PYTHON\tROCKS") == ["python", "rock"]
    assert process_text("TextWith123AndSymbols!@#") == ["textwithandsymbol"]
    assert process_text("lowercase remains unchanged") == ["lowercas", "remain", "unchang"]
    assert process_text("Hello World!123") == ["hello", "world"]
    assert process_text("Text\n\nWith\n\nNewlines and") == ["text", "newlin"]
    assert process_text("Multiple!@#$%^&*() Symbols") == ["multipl", "symbol"]
    assert process_text("Whitespace\t\t\t and Newline\n\n\n") == ["whitespac", "newlin"]
    assert process_text("   Leading Spaces Only   ") == ["lead", "space"]
    assert process_text("Special_characters_are_removed!") == ["specialcharactersareremov"]
    assert process_text("Hello---World!!!") == ["helloworld"]
    assert process_text("1234\n5678\n") == []
    assert process_text("Mixed_Characters123") == ["mixedcharact"]
    assert process_text("End_Of_String!!!") == ["endofstr"]
    assert process_text("A**Test") == ["atest"]
    assert process_text("Another_Test@Example") == ["anothertestexampl"]
    assert process_text("Trailing___Underscores___") == ["trailingunderscor"]
    assert process_text("Larger\nBlock\nOf\nText") == ["larger", "block", "text"]
